...
<!-- live starts -->
_Fetched 2026-08-30 09:47 UTC._

- **go-tsdb** holds 170,760 samples across 30 series, up 23 h; Prometheus is scraping it and the box it runs on is at load 0.45.
- **jlsm** has taken 139.0 M writes and 139.0 M reads from a `redis-benchmark` sidecar in 22 h, through 1,692 flushes and 423 compactions; the Bloom filters turned away 11.2 M disk lookups.
- **jgate** has seen 165 requests: 116 proxied, 39 rate-limited by the Lua token bucket, 10 rejected for a bad JWT.
- **rtmp-server** is muxing an ffmpeg test stream into MPEG-TS: 5 segments in the playlist, sequence number 40,016.
